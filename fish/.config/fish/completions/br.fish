...
complete -c br -l color -d 'Whether to have styles and colors (auto is default and usually OK)' -r -f -a "{auto	,yes	,no	}"
...
complete -c br -l set-install-state -d 'Where to write the produced cmd (if any)' -r -f -a "{undefined	,refused	,installed	}"
...
complete -c br -l send -d 'A socket that broot sends commands to before quitting' -r
complete -c br -l help -d 'Print help information'
complete -c br -s V -l version -d 'Print version information'
