...
complete -c broot -l color -d 'Whether to have styles and colors (auto is default and usually OK)' -r -f -a "{auto	,yes	,no	}"
...
complete -c broot -l set-install-state -d 'Where to write the produced cmd (if any)' -r -f -a "{undefined	,refused	,installed	}"
...
complete -c broot -l send -d 'A socket that broot sends commands to before quitting' -r
complete -c broot -l help -d 'Print help information'
complete -c broot -s V -l version -d 'Print version information'
